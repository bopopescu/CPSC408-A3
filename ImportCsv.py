...
data = data[1:] #strip off headers. unnecessary except for viewing



for match in data:
# separate data into multiple lists for cleaner parsing. *Hard coded indices because it will be the same every time*
    matchDetails = match[0:5]
    scoreboard = match[5].strip('\n').split('\n')
    for idx,x in enumerate(scoreboard):
        scoreboard[idx] = x.split(',')
    performance = match[6].strip('\n').split('\n')
    for idx, x in enumerate(performance):
        performance[idx] = x.split(',')
    timeline = match[7].strip('\n').split('\n')
    for idx,x in enumerate(timeline):
        timeline[idx] = x.split(',')
